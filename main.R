  ...
  dashboardBody(
    ...
    tabItems(
      ...
      tabItem(tabName = "data_explore",
              fluidRow(
                box(title = "Dataset Overview", status = "info", solidHeader = TRUE, uiOutput("dataset_summary")),
                box(title = "Column Details", status = "warning", solidHeader = TRUE, DTOutput("column_details"))
              ),
              fluidRow(
                box(title = "Data Preview", status = "success", solidHeader = TRUE, DTOutput("data_preview")),
                box(title = "Data Statistics", status = "primary", solidHeader = TRUE, uiOutput("data_statistics"))
              )
      ),
...
server <- function(input, output, session) {
  ...
  output$piper_plot <- renderPlot({
    ...
    water_colors <- c(
      "Permanent Hardness" = "#d73027",      # deep red
      "Temporary Hardness" = "#fc8d59",      # orange-red
      "Alkali Carbonates"  = "#91cf60",      # lime green
      "Saline"             = "#4575b4",      # deep blue
      "Mixed Type"         = "#fee08b",      # pale yellow
      "Unable to compute"  = "#bdbdbd"       # light gray
    )
